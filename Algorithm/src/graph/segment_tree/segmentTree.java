...
public class segmentTree {
	...
	/**
	 * 세그먼트 트리는 
	 *  배열의 특정 구간에 대한 연산 저장에 특화된 트리이다.
	 *  특정 구간에 대한 연산값을 조회하는데 O(logN)의 시간복잡도를 가진다.
	 *  
	 * 세그먼트 트리는 이진트리 구조이다.
	 * 현재 범위를 양분하여 양 자식 노드로 가진다.
	 *  -> 왼쪽 자식 : start ~ mid 
	 *  -> 오른쪽 자식 : mid+1 ~ end
	 *  
	 ...
	 *  
	 *  구현)
	 *  이진트리이기 때문에 일차원 배열로 구현이 가능하다.
	 *  배열의 크기는?
	 *   -> 최대의 경우 마지막 레벨에 크기 1인 구간 N개가 들어가기 때문에 
	 *   
	 *  h = ceil(log2(N)); 가 되고 (트리 마지막 레벨)
	 *  2^(h+1) 크기의 배열을 생성해주어야 한다.
	 */
